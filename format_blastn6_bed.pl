...
while (<INPUT>) {
	...
	if ($tmp[9]>$tmp[8]) {# + strand
		print OUTPUT "$tmp[1]\t$tmp[8]\t$tmp[9]\t$tmp[3]\_$tmp[2]\t$tmp[11]\t\+\n";
	}else{
		print OUTPUT "$tmp[1]\t$tmp[9]\t$tmp[8]\t$tmp[3]\_$tmp[2]\t$tmp[11]\t\-\n";
	}
	$seqid{$tmp[0]}=0;
}
close INPUT;
...
sub usage {
    die(
        qq/
Usage:    format_blastn6_bed.pl -i blastn table format -o bed
Function: format blastn table format to bed
